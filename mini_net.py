...
def present_groups(groups):
    """Drop groups naming interfaces this machine doesn't have, so one shared
    config can serve every host without drawing empty bars on the wrong one."""
    if not groups:
        return {}
    here = {name for name, _, _ in _read_raw()}
    return {label: list(ifaces) for label, ifaces in groups.items()
            if here.intersection(ifaces)}
...
def _default_groups():
    ...
    try:
        ...
        return {}
    return present_groups(cfg.get("groups"))
